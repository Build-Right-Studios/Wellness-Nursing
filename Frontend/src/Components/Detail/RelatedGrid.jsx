...
import { getImageUrl } from "../../Pages/Detail";
...
const RelatedGrid = ({ currentId, relatedIds, type }) => {
  ...
  // 1. Get raw items
  const rawRelated = relatedIds?.length
   ? source.filter(s => relatedIds.includes(s.id)).slice(0, 3)
    : source.filter(s => s.id!== currentId).slice(0, 3);

  // 2. Resolve images - THIS IS THE FIX
      ...
      <div className="max-w-[1120px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {related.map(item => (
            <div
              key={item.id}
              onClick={() => navigate(`/${type === "service"? "services" : "equipment"}/${item.id}`)}
              className="bg-white rounded-[12px] overflow-hidden border border-[#EEF3F5] shadow-sm cursor-pointer group"
            >
              <div className="h-[180px] overflow-hidden bg-gray-100">
                <img
                  ...
                  onError={(e) => {
                    e.target.src = `https://placehold.co/600x400?text=${item.name}`;
                  }}
                />
              </div>
              <div className="p-4">
